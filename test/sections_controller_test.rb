...
class SectionsControllerTest < Minitest::Test
  ...
  def test_index_assigns_sections_and_filters_admin_items_by_search_query
    ...
    sections = [ResolvedSection.new(key: "users", title: "Users", subtitle: nil, icon: nil, url: "/admin/sections/users")]
    items = [
      ResolvedItem.new(type: :section, key: "users", title: "Users", subtitle: "People", icon: nil, url: "/admin/sections/users", search_text: "section users people"),
      ResolvedItem.new(type: :screen, key: "user_sign_ins", title: "User sign-ins", subtitle: "Access", icon: nil, url: "/admin/screens/user_sign_ins", search_text: "screen user sign-ins access", parent_key: "users"),
      ResolvedItem.new(type: :screen, key: "user_reviews", title: "User reviews", subtitle: "Reviews", icon: nil, url: "/admin/screens/user_reviews", search_text: "screen user reviews reviews", parent_key: "users"),
      ResolvedItem.new(type: :screen, key: "api_requests", title: "API requests", subtitle: "Traffic", icon: nil, url: "/admin/screens/api_requests", search_text: "screen api requests traffic")
    ]
    ...
    assert_equal ["In Users"], controller.instance_variable_get(:@search_result_groups).map { |group| group.fetch(:title) }
    assert_equal %w[user_sign_ins user_reviews], controller.instance_variable_get(:@search_result_groups).first.fetch(:items).map(&:key)
  end
  ...
  def test_show_uses_surface_root_section_when_key_is_absent
    ...
    with_singleton_stub(RecordingStudioAdmin, :resolve_section, lambda { |key:, context:|
      resolved_key = key
  ...
  def build_controller(params:)
    RecordingStudioAdmin::SectionsController.new.tap do |controller|
